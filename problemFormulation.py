...
class Problem():
    ...
    def __init__(self, verticesDict, launchesDict, verticesMapping):
        ...
        self.initialState = [[],[],0]
        self.actions = ['Choose Vertice', 'Choose Launch'] # maybe dont needed
    ...
    def chooseAction(self, state):
        actions = []
        if not state[1]:
            actions = list(self.launchesDict.keys())
        else:
            if not state[0]:
                actions = list(self.verticesDict.keys())
    ...
    def constrainWeight(self, newState, action):
        if self.launchesDict[list(newState.keys())[-1]][1] > self.verticesDict[action][0]:
            newState[list(newState.keys())[-1]].extend([action])
        return newState

    def constrainLaunches(self, newState, action):
        pass

    def computeCost(self, state, action):
        if action[0] == 'V':
            return self.verticesDict[action][0]
        elif action[0] == 'L':
            return self.launchesDict[action][2]
